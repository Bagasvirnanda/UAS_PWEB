...
if (isset($_POST['mark_read']) && !empty($_POST['notification_ids'])) {
    try {
        ...
        $stmt = $pdo->prepare("UPDATE notifications SET status = 'read' WHERE id IN ($ids)");
        $stmt->execute();
...
if (isset($_POST['delete']) && !empty($_POST['notification_ids'])) {
    try {
        $ids = implode(',', array_map('intval', $_POST['notification_ids']));
        $stmt = $pdo->prepare("DELETE FROM notifications WHERE id IN ($ids)");
        $stmt->execute();
                        ...
                        <tbody>
                            <?php foreach ($notifications as $notification): ?>
                                <tr class="<?php echo $notification['status'] === 'unread' ? 'table-light' : ''; ?>">
                                    ...
                                    <td><?php echo htmlspecialchars($notification['username'] ?? 'System'); ?></td>
